...
function pandown() {
    ...
    for (var i = 0; i < pandowns.length; i++) {
        ...
        if (type == 'baidu') {
            ...
            pandowns[i].innerHTML = '<div><p class="pan-fname" style="color: #000000">' + pandowns[i].getAttribute('fname') + '</p><br><stong class="pan-fintroduce">csdn链接：' + pandowns[i].getAttribute('url') + '</stong><br></div><div><button class="pan-down" style="background-color: #000000" title="立即下载" onclick="copyText(`'+ pandowns[i].getAttribute('pwd') + '`,`' + pandowns[i].getAttribute('url') + '`);">立即下载</button></div>';
        } else if (type == 'yun') {
